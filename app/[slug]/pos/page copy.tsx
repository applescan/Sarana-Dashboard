...
const POS = () => {
    ...
    const handleCreateOrder = async () => {
        ...
        await createOrder({ variables: { orders: [{ totalAmount: order.reduce((acc, item) => acc + item.price * item.quantity, 0), orderItems }] } });
        setOrder([]);
    };
    ...
    return (
        <div>
            ...
            <section>
                <h2>Record Items Sold</h2>
                <input type="text" placeholder="Product ID" value={itemsSold.productId} onChange={(e) => setItemsSold({ ...itemsSold, productId: e.target.value })} />
                <input type="number" placeholder="Quantity" value={itemsSold.quantity} onChange={(e) => setItemsSold({ ...itemsSold, quantity: parseInt(e.target.value) })} />
                <button onClick={handleRecordItemsSold}>Record Items Sold</button>
            </section>

            <section>
                <h2>Record Items Restocked</h2>
                <input type="text" placeholder="Product ID" value={itemsRestocked.productId} onChange={(e) => setItemsRestocked({ ...itemsRestocked, productId: e.target.value })} />
                <input type="number" placeholder="Quantity" value={itemsRestocked.quantity} onChange={(e) => setItemsRestocked({ ...itemsRestocked, quantity: parseInt(e.target.value) })} />
                <button onClick={handleRecordItemsRestocked}>Record Items Restocked</button>
            </section>

            <section>
                <h2>Record Revenue</h2>
                <input type="number" placeholder="Amount" value={newRevenue.amount} onChange={(e) => setNewRevenue({ ...newRevenue, amount: parseFloat(e.target.value) })} />
                <input type="date" placeholder="Date" value={newRevenue.date} onChange={(e) => setNewRevenue({ ...newRevenue, date: e.target.value })} />
                <button onClick={handleRecordRevenue}>Record Revenue</button>
            </section>
